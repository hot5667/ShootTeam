...
export default function RootLayout({
  ...
  return (
    <html lang="ko" className="h-full">
      <body className="bg-[#1e1e1e] text-white font-mono flex flex-col min-h-full">
        <header className="bg-[#2d2d2d] text-white py-4 shadow-md border-b border-[#333]">
          <div className="max-w-7xl mx-auto flex justify-between items-center px-4 sm:px-8">
            <div className="flex flex-col items-center sm:items-start">
              <Link href="/" passHref>
                <Image src="/logo.png" alt="새싹 게임즈 로고" width={40} height={40} />
              </Link>
            </div>

            <button onClick={toggleSidebar} className="sm:hidden text-white text-2xl">
              &#9776;
            </button>
        ...
        {isSidebarOpen && (
          <div className="sm:hidden fixed inset-0 bg-black bg-opacity-50 z-10">
            <div className="fixed right-0 top-0 w-[300px] h-full bg-gradient-to-r from-[#333] to-[#1e1e1e] text-white py-8 flex flex-col items-center space-y-6 transform transition-transform duration-300 ease-in-out">
              <Image src="/logo.png" alt="새싹 게임즈 로고" width={40} height={40} className="mb-8" />

              <Link href="/" className="text-lg w-full text-center py-2 hover:bg-[#444] transition-colors duration-200 rounded-md" onClick={toggleSidebar}>
                홈
              </Link>

              <div className="w-full text-center">
                <button onClick={toggleMobileAccordion} className="text-lg w-full py-2 hover:bg-[#444] transition-colors duration-200 rounded-md">
                  팀원 소개
                </button>
                {isMobileAccordionOpen && (
                  <motion.div className="overflow-hidden bg-[#444] rounded-md shadow-md mt-2">
                    <ul className="py-2">
                      <li className="p-3 text-sm hover:bg-[#555] transition-colors">
                        <Link href="/team/kim-dohyun">김도현</Link>
                      </li>
              ...
              <Link href="/game" className="text-lg w-full text-center py-2 hover:bg-[#444] transition-colors duration-200 rounded-md" onClick={toggleSidebar}>
                게임
              </Link>

              <button onClick={toggleSidebar} className="absolute top-6 left-6 text-3xl text-white">&times;</button>
            </div>
          </div>
        )}

        <main className="flex-1">
          <div className="px-4 sm:px-8 py-10 rounded-lg mx-auto border-[#333]">{children}</div>
        </main>

        <footer className="bg-[#2d2d2d] text-white py-4 border-t border-[#333] mt-auto">
